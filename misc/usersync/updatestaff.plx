...
# Reports
#
open( REPORT, ">$report_directory/UpdateStaff_$today.log" );
print REPORT "Reporting results of Registry Import - Update Staff Operation \n\n";
...
$twig->parsefile('/home/syncuser/imports/staff_update.xml');  # Parse XML File
...
my $subject = "'" . "Staff Registry Update Report " . "$today" . "'";
my $message = "$report_directory/UpdateStaff_$today.log";
system "mutt -s $subject $adminemail < $message";
# Move file to backup
system(
    "sudo", "mv",
    "/home/syncuser/imports/staff_update.xml",
    "/home/syncuser/imports/past/staff_update_$today.xml"
);
...
sub EMPLOYEE {
    ...
    $title     = $EMPLOYEE->first_child('TITLE');      # borrowers TITLE
    $surname   = $EMPLOYEE->first_child('SURNAME');    # borrowers SURNAME
    $forenames = $EMPLOYEE->first_child('FORENAMES')
      ;    # borrowers FIRSTNAME OTHERNAMES (XML Concatenated by spaces)
    $dob   = $EMPLOYEE->first_child('DOB');     # borrowers DOB
    $pin   = $EMPLOYEE->first_child('PIN');     # borrowers PASSWORD (MD5 of PIN)
    $btype = $EMPLOYEE->first_child('BTYPE');   # borrowers CATEGORYCODE
    $barcode  = $EMPLOYEE->first_child('BARCODE');    # borrowers CARDNUMBER
    $mifare   = $EMPLOYEE->first_child('MIFARE');     # borrower_attributes SMART
    $username = $EMPLOYEE->first_child('USERNAME');   # borrowers USERID
    $email    = $EMPLOYEE->first_child('EMAIL');      # borrowers EMAIL
    $phone    = $EMPLOYEE->first_child('TELEPHONE');  # borrowers PHONE
    $mobile = $EMPLOYEE->first_child('MOBILE_TELEPHONE');    # borrowers MOBILE
    $location = $EMPLOYEE->first_child('LOCATION');    # borrowers BRANCHCODE
    $course_code = $EMPLOYEE->first_child('COURSE'); # borrower_attributes COURSE
    $level      = $EMPLOYEE->first_child('STUDY_LEVEL');          # UNUSED
    $address1_1 = $EMPLOYEE->first_child('PRIMARY_ADDRESS_1');    #
    $address2_1 = $EMPLOYEE->first_child('PRIMARY_ADDRESS_2');    #
    $address3_1 = $EMPLOYEE->first_child('PRIMARY_ADDRESS_3');    #
    $address4_1 = $EMPLOYEE->first_child('PRIMARY_ADDRESS_4');    #
    $address5_1 = $EMPLOYEE->first_child('PRIMARY_ADDRESS_4');    #
    $postcode_1 = $EMPLOYEE->first_child('PRIMARY_POSTCODE'); # borrowers ZIPCODE
    $address1_2 = $EMPLOYEE->first_child('SECONDARY_ADDRESS_1');  # Other Address
    $address2_2 = $EMPLOYEE->first_child('SECONDARY_ADDRESS_2');  #
    $address3_2 = $EMPLOYEE->first_child('SECONDARY_ADDRESS_3');  #
    $address4_2 = $EMPLOYEE->first_child('SECONDARY_ADDRESS_4');  #
    $address5_2 = $EMPLOYEE->first_child('SECONDARY_POSTCODE');   #
    $postcode_2 =
